...
from questions.payments.payments import AsyncStripeClient, stripe_keys
...
def delete_customer_from_database(email: str, db_name: str) -> bool:
    ...
    try:
        ...
        result = session.execute(
            text("SELECT id, stripe_id FROM users WHERE email = :email"),
            {"email": email}
        ).fetchone()
        ...
        try:
            session.execute(
                text("DELETE FROM chat_rooms WHERE user_id = :user_id"),
                {"user_id": user_id}
            )
        except Exception:
            pass  # Table might not exist in older schema
        ...
        try:
            session.execute(
                text("DELETE FROM save_games WHERE user_id = :user_id"),
                {"user_id": user_id}
            )
        except Exception:
            pass  # Table might not exist in older schema
