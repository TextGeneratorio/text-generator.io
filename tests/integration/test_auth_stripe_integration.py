...
sys.modules["sellerinfo"] = types.SimpleNamespace(
    STRIPE_LIVE_SECRET="sk_test_mock", STRIPE_LIVE_KEY="pk_test_mock", CLAUDE_API_KEY="test_key"
)
...
class TestAuthStripeIntegration:
    ...
    def test_login_with_invalid_stripe_id_recreates_customer(self):
        ...
        mock_error = stripe.error.InvalidRequestError(
            f"No such customer: '{self.invalid_stripe_id}'", param="id", code="resource_missing"
        )
        ...
        with (
            patch("stripe.Customer.retrieve", side_effect=mock_error),
            ...
            response = client.post("/api/login", data={"email": self.test_email, "password": self.test_password})
    ...
    def test_current_user_api_with_invalid_stripe_id(self):
        ...
        with (
            patch("stripe.Customer.retrieve", return_value=mock_valid_customer),
            patch("questions.payments.payments.get_subscription_item_id_for_user_email", return_value=None),
        ):
            login_response = client.post("/api/login", data={"email": self.test_email, "password": self.test_password})
    ...
    def test_new_user_signup_creates_stripe_customer(self):
        ...
        with (
            ...
            response = client.post("/api/login", data={"email": "newuser@example.com", "password": "newpassword123"})
    ...
    def test_stripe_customer_creation_failure_handling(self):
        ...
        with (
            ...
            response = client.post("/api/login", data={"email": "failuser@example.com", "password": "failpassword123"})
    ...
    def test_various_stripe_errors_during_login(self, stripe_error):
        ...
        with (
            patch("stripe.Customer.retrieve", side_effect=stripe_error),
