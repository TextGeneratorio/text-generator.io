...
from routes.documents import autosave_document_route as autosave_document
from routes.documents import get_document_route as get_document
from routes.documents import list_documents_route as list_documents
from routes.documents import save_document_route as save_document
...
async def test_list_documents_route(db_session, test_user, test_document):
    ...
    doc2 = Document(id=doc2_id, user_id=test_user.id, title="Second Document", content="More content")
    db_session.add(doc2)
    db_session.commit()
...
async def test_save_document_route_new(db_session, test_user):
    ...
    request = MockRequest(
        json_data={"title": "New Document", "content": "New content"}, cookies={"session_secret": test_user.secret}
    )
    with patch("routes.documents.get_current_user", return_value=test_user):
...
async def test_save_document_route_update(db_session, test_user, test_document):
    ...
    request = MockRequest(
        json_data={"id": test_document.id, "title": "Updated Document", "content": "Updated content"},
...
async def test_save_document_route_permission_denied(db_session, test_user, test_document):
    ...
    request = MockRequest(
        json_data={"id": test_document.id, "title": "Malicious Update", "content": "Malicious content"},
...
async def test_save_document_route_missing_user_id(db_session):
    ...
    request = MockRequest(json_data={"title": "Document without user", "content": "Content"}, cookies={})
    with patch("routes.documents.get_current_user", return_value=None):
        response = await save_document(request, db_session)
...
async def test_autosave_document_route(db_session, test_user):
    ...
    request = MockRequest(
        json_data={"title": "Autosaved Document", "content": "Autosaved content"},
        cookies={"session_secret": test_user.secret},
    )
    with patch("routes.documents.get_current_user", return_value=test_user):
        ...
        doc_id = data["id"]
        request = MockRequest(
            json_data={"id": doc_id, "title": "Updated Autosave", "content": "Updated autosave content"},
            cookies={"session_secret": test_user.secret},
        )
        response = await autosave_document(request, db_session)
