...
def test_get_modernbert_uses_checkpoint(monkeypatch):
    calls = {}
    monkeypatch.setattr(
        "transformers.AutoModel.from_pretrained", lambda cp: (_fake_model(), calls.setdefault("cp", cp))[0]
    )
    monkeypatch.setattr("transformers.AutoTokenizer.from_pretrained", lambda cp: _fake_tokenizer)
    bert_embed = (
        importlib.reload(sys.modules.get("questions.bert_embed"))
        if "questions.bert_embed" in sys.modules
        else importlib.import_module("questions.bert_embed")
    )
