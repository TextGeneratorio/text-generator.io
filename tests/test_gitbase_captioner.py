...
class TestGitBaseCaptioner:
    ...
    def test_initialization(self):
        ...
        captioner = GitBaseCaptioner(
            model_name="custom/model", dtype=torch.float32, device="cpu", use_compile=False, use_channels_last=False
        )
    ...
    def test_load_model(self, mock_model, mock_processor, captioner):
        ...
        mock_model.assert_called_once_with("microsoft/git-base", torch_dtype=torch.float32)
    ...
    def test_caption_image(self, mock_model, mock_processor, captioner, sample_image):
        ...
        mock_processor_instance.batch_decode.assert_called_with(
            torch.tensor([[101, 102, 103]]), skip_special_tokens=True
        )
...
class TestModuleFunctions:
    ...
    def test_get_gitbase_captioner(self, mock_cache):
        ...
        # Test function call
        result = get_gitbase_captioner(dtype=torch.float16, use_compile=True, use_channels_last=False)
...
class TestPerformanceOptimizations:
    ...
    def test_optimization_flags(self):
        ...
        captioner_optimized = GitBaseCaptioner(use_compile=True, use_channels_last=True)
        assert captioner_optimized.use_compile is True
        assert captioner_optimized.use_channels_last is True

        # Test with optimizations disabled
        captioner_basic = GitBaseCaptioner(use_compile=False, use_channels_last=False)
...
class TestCUDAOptimizations:
    ...
    def test_channels_last_with_cuda(self):
        ...
        captioner = GitBaseCaptioner(device="cuda", use_channels_last=True)
        assert captioner.use_channels_last is True
