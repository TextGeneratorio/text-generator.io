...
LOG_FILE = "/nvme0n1-disk/code/text-generator.io/monitors/monitor_site.log"
LOCK_FILE = "/tmp/text-generator-monitor-site.lock"
...
def check_site(url: str, timeout: int = 10) -> bool:
    try:
...
def acquire_lock():
    try:
        ...
        log("Another monitor_site instance is running; exiting.")
        return None
...
def main():
    ...
    last_restart_site = 0.0
    last_restart_cloudflared = 0.0
    local_failures = 0
    external_failures = 0

    while True:
        local_ok = check_site(URL_LOCAL)
        if not local_ok:
            local_failures += 1
            external_failures = 0
            log(f"LOCAL DOWN ({local_failures}/{FAIL_THRESHOLD})")
            if local_failures >= FAIL_THRESHOLD and time.time() - last_restart_site > RESTART_COOLDOWN:
                restart_systemd_service("text-generator-site")
                last_restart_site = time.time()
                local_failures = 0
        else:
            local_failures = 0
            external_ok = check_site(URL_EXTERNAL)
            if external_ok:
                external_failures = 0
                ...
                if external_failures >= FAIL_THRESHOLD and time.time() - last_restart_cloudflared > RESTART_COOLDOWN:
                    restart_supervisor_service("cloudflared-site")
