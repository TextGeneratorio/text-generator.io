...
# Use production database by default (textgen_prod)
PRODUCTION_DB_NAME = "textgen_prod"
FALLBACK_DB_NAME = "textgen"

# Construct production database URL
PROD_DATABASE_URL = f"postgresql://{PRODUCTION_DB_USER}:{PRODUCTION_DB_PASSWORD}@{PRODUCTION_DB_HOST}:{PRODUCTION_DB_PORT}/{PRODUCTION_DB_NAME}"
FALLBACK_DATABASE_URL = f"postgresql://{PRODUCTION_DB_USER}:{PRODUCTION_DB_PASSWORD}@{PRODUCTION_DB_HOST}:{PRODUCTION_DB_PORT}/{FALLBACK_DB_NAME}"

# Try production database first, fallback to regular textgen database
DATABASE_URL = os.getenv("DATABASE_URL", PROD_DATABASE_URL)

# If no custom DATABASE_URL is set, always prefer production
...
def create_engine_with_fallback():
    ...
    for prod_url, fallback_url, conn_type in [
        ...
        try:
            ...
            print(f"✅ Connected to production database: {PRODUCTION_DB_NAME} ({conn_type} connection)")
            return engine, prod_url
        except Exception as e:
            print(f"⚠️  Production database ({PRODUCTION_DB_NAME}) {conn_type} connection failed: {e}")
            try:
                # Fallback to regular textgen database
                engine = create_engine(fallback_url)
                # Test the connection
                with engine.connect() as conn:
                    conn.execute(text("SELECT 1"))
                print(f"✅ Connected to fallback database: {FALLBACK_DB_NAME} ({conn_type} connection)")
...
try:
    ...
    print(f"❌ All database connections failed, using default configuration: {e}")
    engine = create_engine(DATABASE_URL)
