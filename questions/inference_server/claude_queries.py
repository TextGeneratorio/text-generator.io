...
async def query_to_claude_async(
    ...
    try:
        ...
        truncated_system = ""
        if system_message:
            truncated_system = truncate_to_max_tokens(
                system_message, max_tokens=2000
            ).strip()
            ...
            try:
                message = await asyncio.wait_for(
                    claude_client.messages.create(
                        ...
                        stop_sequences=(
                            [s for s in list(stop_sequences) if s and s.strip()]
                            if stop_sequences
                            else []
                        ),
                    ),
                ...
                if "too many tokens" in str(token_error).lower():
                    logger.error(
                        f"Token limit exceeded in Claude async query: {token_error}"
                    )
                    # Try again with more aggressive truncation
                    more_truncated_prompt = truncate_to_max_tokens(
                        prompt, max_tokens=40000
                    )
                    messages = [
                        {
                            "role": "user",
                    ...
                    if message.content:
                        generated_text = message.content[0].text
                        logger.info(
                            f"Claude Generated text (after retry): {len(generated_text)}"
                        )
                        return generated_text
                    return None
                else:
...
async def query_to_claude_image_async(
    prompt: str,
    image_bytes: bytes,
    system_message: Optional[str] = None,
    model: Optional[str] = "claude-sonnet-4-20250514",
) -> Optional[str]:
    """Call Claude with an image and text prompt."""
